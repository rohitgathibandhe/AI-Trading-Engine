"""
Credit spread simplified backtest simulation.

Simple deterministic model similar to others.
"""
...
class Strategy:
    ...
    def run_backtest(self, df=None, cfg=None, *args, **kwargs) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, Any]]:
        ...
        min_credit = float(cfg.get("min_credit_per_lot", cfg.get("min_total_credit_per_lot", 2000.0)))
        ...
        for _, r in sells.iterrows():
            ...
            premium = min_credit * prob
            move = abs((exit_close - entry_close) / entry_close)
            if move <= 0.02:
                pnl = premium * 0.6
            else:
                pnl = -min(premium * 8.0, move * (self.capital or cfg.get("capital", 1e5)))
            trades.append({"entry_date": entry_date, "exit_date": exit_date, "premium": premium, "pnl": pnl, "prob": prob})
